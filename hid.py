...
class USBHID(USBDevice):
    ...
    def send_mouse(self, x:int, y:int, z:int):
        if self.usb_req is not None:
            self.send_usb_req(self.usb_req, bytes([0,x,y,z]))
...
import threading
t0 = threading.Thread(target=usb_job, daemon=True)
t0.start()

i = 0
while True:
    input()
    usb_Dev.send_mouse(1, 0, 0)
    i+=1
    i &= 0xff
